...
function load_map(json, customOption) {
  ...
  L.tileLayer(
    "https://api.mapbox.com/styles/v1/mapbox/streets-v11/tiles/{z}/{x}/{y}?access_token=pk.eyJ1IjoibGluYTAwNyIsImEiOiJja2hmZDNvOTgwNnVrMnJsNG1sOWtzcXNoIn0.Do0MCp-8-o2cv5cl-A2sNQ",
    {
      attribution:
  ...
  try {
    ...
    mymap.addControl(
      new L.Control.Search({
        ...
        moveToLocation: function (latlng, title, map) {
          ...
          let targetmarker = markers.find(
            (el) => el.defaultOptions.title === title
          );
    ...
    // filter modal button for mobile
    L.easyButton({
...
function addPopup(feature, layer) {
  var popupText =
    "<p id='p_popup_detail'>" +
    "<strong style='color: #84b819; font-size:120%;' >" +
    feature.properties.project_name_wb +
    "</strong><br>" +
    "<b>Country:</b> " +
    feature.properties.country +
    "<br>" +
    "<b>Income Group:</b> " +
    feature.properties.income_group +
    "<br>" +
    "<b>FC Year:</b> " +
    feature.properties.fc_year +
    "<br>" +
    "<b>Status:</b> " +
    feature.properties.ppi_status +
    "<br>" +
    "<b>Primary Sector:</b> " +
    feature.properties.sector +
    "<br>" +
    "<b>Sub Sector:</b> " +
    feature.properties.subsector +
    "<br>" +
    "<b>Problem:</b> " +
    feature.properties.reason_for_delay +
    "<br>" +
    "<b>Type of PPI:</b> " +
    feature.properties.type_of_ppi +
    "<br>" +
    "<p id='linked_p_popup_detail'>" +
    "<b><a href='" +
    feature.properties.urls +
...
function geoJson_filter(feature) {
  ...
  const countryselect =
    customOption.countryOp.length == 0
      ? true
      : customOption.countryOp.includes(feature.properties.country);
  const sectorselect =
    customOption.sectorOp.length == 0
      ? true
      : sectorClass(customOption.sectorOp, feature.properties);
  const statusselect =
    customOption.statusOp.length == 0
      ? true
      : customOption.statusOp.includes(feature.properties.ppi_status);
  const incomeselect =
    customOption.incomeOp.length == 0
      ? true
      : customOption.incomeOp.includes(feature.properties.income_group);
  const ppitypeselect =
    customOption.ppitypeOp.length == 0
      ? true
      : customOption.ppitypeOp.includes(feature.properties.type_of_ppi);
...
function detectChange(json) {
  for (let input of document.querySelectorAll(
    ".select, .js-range-slider, #myCheck"
  )) {
...
function toggle_selectableOptgroup() {
  ...
  $(document).on(
    "click",
    "#select2-country-select-results strong",
    function () {
